...
# Loop to input data for each appliance
for i in range(num_appliances):
    st.subheader(f"Appliance {i+1}")
    name = st.text_input(f"Name of Appliance {i+1}", key=f"name_{i}")
    power_watts = st.number_input(f"Power Rating (Watts) for {name or f'Appliance {i+1}'}", min_value=1.0, value=100.0, key=f"power_{i}")
    usage_hours = st.number_input(f"Daily Usage (Hours) for {name or f'Appliance {i+1}'}", min_value=0.0, value=1.0, key=f"hours_{i}")
    quantity = st.number_input(f"Quantity of {name or f'Appliance {i+1}'}", min_value=1, value=1, key=f"qty_{i}")

    daily_kwh = (power_watts * usage_hours * quantity) / 1000
    total_daily_kwh += daily_kwh

    appliance_data.append({
        "name": name or f"Appliance {i+1}",
...
# Optional electricity cost input
